...
Upload each paid file to the private `product-files` bucket using the exact `storage_path` value configured for that product, for example `ebooks/midjourney-for-architects.pdf` or `ebooks/resource-pack.zip`.
