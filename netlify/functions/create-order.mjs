...
const allowedCurrencies = new Set(['INR', 'USD', 'AED', 'EUR', 'GBP', 'SGD']);
...
const validateGenericOrder = (payload) => {
  ...
  if (!allowedCurrencies.has(currency)) return { error: 'This currency is not enabled for checkout.' };
...
export default async (request) => {
  ...
  if (!allowedCurrencies.has(productCurrency)) {
    return jsonResponse({ error: 'This product currency is not enabled for checkout.' }, 422);
  }
