...
export const AiArchitecture: React.FC<AiArchitectureProps> = ({ 
  ...
  return (
    ...
    >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 items-stretch">
            ...
            <div className="lg:col-span-5 p-6 sm:p-8 flex flex-col justify-between space-y-6 bg-[#11131a]">
              <div className="space-y-5">
                ...
                <div className="rounded-xl bg-[#090a0f] border border-white/10 p-4">
                  <div className="flex items-center justify-between gap-4">
                    <span className="text-[10px] font-sans font-bold uppercase tracking-wider text-[#8e929b]">
                      {activePrompt.type === 'FREE' ? 'Free Resource' : 'Premium Resource'}
                    </span>
                    <PriceDisplay
                      price={activePrompt.type === 'FREE' ? 0 : activePrompt.price || '29'}
                      compareAtPrice={activePrompt.type === 'FREE' ? getFreePromptCompareAtPrice(activePrompt) : activePrompt.compareAtPrice}
                      currency={activePrompt.currency}
                      currentClassName="text-sm font-sans font-extrabold text-white"
                      compareClassName="text-xs text-[#8e929b] line-through"
                      badgeClassName="text-[10px] font-bold text-[#c5a880]"
                    />
                  </div>
                  {hasPromptFile(activePrompt) && (
                    <div className="mt-3 flex items-center gap-2 text-xs font-sans font-bold text-[#e8e9ed]">
                      <FileText className="w-4 h-4 text-[#c5a880]" />
                      <span>PDF file included</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-3 pt-2">
                {activePrompt.type === 'FREE' ? (
                  ...
                  >
                    ...
                    <span className="inline-flex items-baseline gap-1.5">GET FULL PROMPT (<PriceDisplay price={activePrompt.price || '29'} compareAtPrice={activePrompt.compareAtPrice} currency={activePrompt.currency} currentClassName="text-inherit" compareClassName="text-[#090a0f]/60 line-through" badgeClassName="text-[#090a0f]/70" />) →</span>
                  </button>
                )}
      ...
      <AnimatePresence>
        {selectedPromptModal && (
          <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/85 p-3 backdrop-blur-md sm:items-center sm:p-4">
            <div className="fixed inset-0" onClick={() => setSelectedPromptModal(null)} />
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.95 }}
              className="relative z-10 max-h-[calc(100dvh-1.5rem)] w-full max-w-2xl space-y-6 overflow-y-auto rounded-2xl border border-white/20 bg-[#13151c] p-6 shadow-2xl no-scrollbar sm:max-h-[90vh] sm:p-8"
            >
              <div className="sticky top-0 z-20 -mx-6 -mt-6 flex items-center justify-between border-b border-white/10 bg-[#13151c]/95 px-6 pb-3 pt-6 backdrop-blur sm:-mx-8 sm:-mt-8 sm:px-8 sm:pt-8">
              ...
              <div className="space-y-3">
                ...
                <div className="p-4 rounded-xl bg-[#090a0f] border border-white/10 space-y-3">
                  <div className="flex items-center justify-between gap-4">
                    <span className="text-[10px] font-sans font-bold text-[#c5a880] uppercase tracking-wider">
                      {selectedPromptModal.type === 'FREE' ? 'Free Resource' : 'Premium Resource'}
                    </span>
                    <PriceDisplay
                      price={selectedPromptModal.type === 'FREE' ? 0 : selectedPromptModal.price || '29'}
                      compareAtPrice={selectedPromptModal.type === 'FREE' ? getFreePromptCompareAtPrice(selectedPromptModal) : selectedPromptModal.compareAtPrice}
                      currency={selectedPromptModal.currency}
                      currentClassName="text-sm font-sans font-extrabold text-white"
                      compareClassName="text-xs text-[#8e929b] line-through"
                      badgeClassName="text-[10px] font-bold text-[#c5a880]"
                    />
                  ...
                  {hasPromptFile(selectedPromptModal) && (
                    <div className="flex items-center gap-2 text-xs font-sans font-bold text-[#e8e9ed]">
                      <FileText className="w-4 h-4 text-[#c5a880]" />
                      <span>PDF file included</span>
                    </div>
                  )}
              ...
              <div className="flex flex-col justify-end gap-3 pt-2 sm:flex-row">
                ...
                {selectedPromptModal.type === 'FREE' ? (
                  ...
                  >
                    ...
                    <span className="inline-flex items-baseline gap-1.5">BUY PROMPT (<PriceDisplay price={selectedPromptModal.price || '29'} compareAtPrice={selectedPromptModal.compareAtPrice} currency={selectedPromptModal.currency} currentClassName="text-inherit" compareClassName="text-[#090a0f]/60 line-through" badgeClassName="text-[#090a0f]/70" />)</span>
                  </button>
                )}
              </div>
            </motion.div>
