...
const productDeliveryType = (item: any, storagePath: string) => {
  if (CHECKOUT_DELIVERY_TYPES.has(item?.deliveryType)) return item.deliveryType;
  const pathname = (() => {
    try { return new URL(storagePath).pathname; } catch { return storagePath; }
  })();
  return /\.pdf(?:$|[?#])/i.test(pathname) ? 'pdf' : 'file';
};
...
export function DataProvider({ children }: { children: React.ReactNode }) {
  ...
  const syncProduct = async (collection: string, item: any, activeOverride?: boolean) => {
    ...
    if (paid && !CHECKOUT_CURRENCIES.has(currency)) {
