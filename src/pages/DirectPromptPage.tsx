...
import { Check, Copy, Download, FileText, Lock, Share2 } from 'lucide-react';
              ...
              <div className="grid grid-cols-1 items-start gap-6 p-5 sm:grid-cols-12 sm:p-8">
                ...
                <div className="space-y-4 sm:col-span-7">
                  <div>
                    ...
                    <p className="mt-1 text-xs font-sans font-medium text-[#747783]">
                      {prompt.workflowStep}
                    </p>
                  </div>

                  <div className="grid grid-cols-2 gap-2 text-[11px] font-sans">
                    <div className="rounded-lg border border-[#12141a]/10 bg-[#faf8f5] p-3">
                      <span className="block font-bold uppercase tracking-wider text-[#747783]">Engine</span>
                      <span className="mt-1 block text-[#12141a]">{prompt.parameters?.engine || 'Midjourney / SDXL'}</span>
                    </div>
                    <div className="rounded-lg border border-[#12141a]/10 bg-[#faf8f5] p-3">
                      <span className="block font-bold uppercase tracking-wider text-[#747783]">Aspect</span>
                      <span className="mt-1 block text-[#12141a]">{prompt.parameters?.aspectRatio || '16:9'}</span>
                    </div>
                  </div>

                  <div className="space-y-2 rounded-xl border border-[#12141a]/10 bg-[#faf8f5] p-4">
                    <div className="text-[10px] font-sans font-bold uppercase tracking-wider text-[#9e825d]">
                      {hasPromptFile(prompt) && prompt.type === 'FREE' ? 'PDF Prompt File' : 'Prompt Matrix Syntax'}
                    </div>
                    {hasPromptFile(prompt) && prompt.type === 'FREE' ? (
                      <div className="flex items-start gap-3 rounded border border-[#12141a]/10 bg-[#ffffff] p-3">
                        <FileText className="mt-0.5 h-5 w-5 shrink-0 text-[#9e825d]" />
                        <div className="space-y-1">
                          <p className="text-xs font-sans font-bold text-[#12141a]">
                            This free prompt is delivered as a downloadable PDF.
                          </p>
                          <p className="text-[11px] font-sans leading-relaxed text-[#747783]">
                            Text copy is hidden because a PDF file has been uploaded for this prompt.
                          </p>
                        </div>
                      </div>
                    ) : (
                      <p className="max-h-44 select-all overflow-y-auto rounded border border-[#12141a]/10 bg-[#ffffff] p-3 font-mono text-xs leading-relaxed text-[#4a4d57] no-scrollbar">
                        {prompt.type === 'FREE'
                          ? prompt.fullPrompt
                          : `${prompt.previewText} ... [premium parameters locked]`}
                      </p>
                    )}
                  </div>
                </div>
