...
export default function PurchaseSuccessPage() {
  ...
  const isDownload = state.deliveryType === 'file' || state.deliveryType === 'pdf' || !state.deliveryType;
          ...
          {hasAccess ? (
            isDownload ? (
              <button
                type="button"
                onClick={async () => {
                  if (!state.accessUrl || isDownloading) return;
                  setIsDownloading(true);
                  await downloadFileFromUrl(state.accessUrl, state.productName);
                  setIsDownloading(false);
                }}
                disabled={isDownloading}
                aria-label={`Download ${state.productName}`}
                className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#12141a] px-5 py-3.5 text-xs font-bold uppercase tracking-[0.14em] text-white transition-colors hover:bg-[#9e825d] disabled:cursor-wait disabled:opacity-70 sm:w-auto"
              >
                <Download className="h-4 w-4" />
                {isDownloading ? 'Preparing Download...' : 'Download Your Product'}
              </button>
            ) : (
              <a href={state.accessUrl} aria-label={`Access ${state.productName}`} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#12141a] px-5 py-3.5 text-xs font-bold uppercase tracking-[0.14em] text-white transition-colors hover:bg-[#9e825d] sm:w-auto">
